...
gulp.task('compile', ['clean'], function() {
  return browserify('lib/bespoke-extern.js', { standalone: 'bespoke.plugins.extern' }).bundle()
    .pipe(source('bespoke-extern.js'))
    .pipe(buffer())
    .pipe(header([
      '/*!',
      ' * <%= name %> v<%= version %>',
    ...
    .pipe(gulp.dest('dist'))
    .pipe(rename('bespoke-extern.min.js'))
    .pipe(uglify())
    .pipe(header([
      '/*! <%= name %> v<%= version %> ',
...
gulp.task('compile:demo:html', ['clean'], function() {
  return gulp.src('demo/index.html')
    .pipe(replace(/<script [\s\S]*?<script src="demo.js"><\/script>/, '<script src="build.js"></script>'))
    .pipe(gulp.dest('demo/dist'));
});
